...
function precioCorrederasExtra(pct,m){
  pct=String(pct||'0'); if(pct==='0') return {sin:0,con:0};
  const arr=correderasExtrasArr[pct]||[]; if(!arr.length) return {sin:0,con:0}; m=Number(m||0);
  if(m>5.5){
    const ext=extensionDesde550(arr,m);
    if(ext) return {sin:ext.sin, con:ext.con};
  }
...
function actualizarVistaPrecio(){
  ...
  const mc=medidaCobradaProducto(prod,variant,m), p=precioBase(prod,variant,m,motorModelo), ce=permiteCorrederas?precioCorrederasExtra(pct,m):{sin:0,con:0};
...
function calcTotals(){
  ...
  for(const t of tramos){
    const p=precioBase(t.producto,t.variante,t.medida,t.motorModelo), ce=esRipplefold(t.producto,t.variante)?precioCorrederasExtra(t.correderasExtraPct,t.medida):{sin:0,con:0};
...
function enviarWhatsapp(){ if(!tramos.length&&!accs.length){alert('Agrega al menos un tramo o accesorio antes de enviar la cotización.');return} if(!validarUnionesCotizacion()) return; const tel=($('telefonoCliente').value||'').replace(/\D/g,''); const texto=encodeURIComponent(textoWhatsapp()); const url=tel?`https://wa.me/52${tel}?text=${texto}`:`https://wa.me/?text=${texto}`; window.open(url,'_blank'); }
