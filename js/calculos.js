...
function correderasPorMetroEspanola(pct){
  return ({'80':20,'100':22,'120':24})[String(pct||'80')] || 20;
}
function cintaEspanolaInfo(prod,m,activa,pct,sep){
  const aplica=esRipplefoldEspanola(prod) && !!activa;
  if(!aplica) return {aplica:false,pct:String(pct||'80'),sep:Number(sep||0.15),correderasMetro:0,broches:0,metros:0,precio:{sin:34.34,con:39.83},total:{sin:0,con:0}};
  const ancho=Number(m||0);
  const porcentaje=String(pct||'80');
