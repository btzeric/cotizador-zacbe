...
function correderasPorMetroEspanola(pct){
  return ({'60':18,'80':20,'100':22,'120':24})[String(pct||'60')] || 18;
}
function cintaEspanolaInfo(prod,m,activa,pct,sep){
  const aplica=esRipplefoldEspanola(prod) && !!activa;
  if(!aplica) return {aplica:false,pct:String(pct||'60'),sep:Number(sep||0.15),correderasMetro:0,broches:0,metros:0,precio:{sin:34.34,con:39.83},total:{sin:0,con:0}};
  const ancho=Number(m||0);
  const porcentaje=String(pct||'60');
  const separacion=Number(sep||0.15);
  const correderasMetro=correderasPorMetroEspanola(porcentaje);
  const broches=ancho*correderasMetro;
  const metros=broches*separacion;
  const precio={sin:34.34,con:39.83};
  return {aplica:true,pct:porcentaje,sep:separacion,correderasMetro,broches,metros,precio,total:{sin:metros*precio.sin,con:metros*precio.con}};
}
...
function actualizarBloqueCintaEspanola(){ const visible=esRipplefoldEspanola($('producto').value); const bloque=$('bloqueCintaEspanola'); if(bloque){ bloque.style.display=visible?'block':'none'; } if(!visible){ if($('cintaEspanolaActiva')) $('cintaEspanolaActiva').checked=false; if($('cintaEspanolaPct')) $('cintaEspanolaPct').value='60'; if($('cintaEspanolaSep')) $('cintaEspanolaSep').value='0.15'; if($('cintaEspanolaVista')) $('cintaEspanolaVista').textContent='$0.00'; } return visible; }
...
function actualizarVistaPrecio(){
  ...
  const permiteCintaEspanola=actualizarBloqueCintaEspanola();
  ...
  const cintaActiva=permiteCintaEspanola && !!$('cintaEspanolaActiva')?.checked;
  const cintaPct=$('cintaEspanolaPct')?.value||'80';
  const cintaSep=parseFloat($('cintaEspanolaSep')?.value||'0.15');
  if($('avisoUnionObligatoria')) $('avisoUnionObligatoria').style.display=requiereUnionPorMedida(m)?'block':'none';
  if(!m||m<=0){$('precioSeleccionado').textContent='Selecciona producto, variante y medida.';$('correderasExtraVista').textContent='$0.00'; if($('cintaEspanolaVista')) $('cintaEspanolaVista').textContent='$0.00'; if($('soportesAutoVista')) $('soportesAutoVista').textContent='$0.00'; return;}
  const mc=medidaCobradaProducto(prod,variant,m), p=precioBase(prod,variant,m,motorModelo), ce=permiteCorrederas?precioCorrederasExtra(pct,m,prod):{sin:0,con:0}, cinta=cintaEspanolaInfo(prod,m,cintaActiva,cintaPct,cintaSep);
  ...
  if($('cintaEspanolaVista')) $('cintaEspanolaVista').innerHTML=cinta.aplica?`Ancho: ${m.toFixed(2)} m · ${cinta.pct}% (${cinta.correderasMetro} correderas/m) · Separación: ${cinta.sep.toFixed(2)}<br>Broches: ${cinta.broches.toFixed(2)} · Metros de cinta: ${cinta.metros.toFixed(2)} m · Total: ${money(precioMostrado(cinta.total))}`:'Activa la casilla para agregar la cinta española.';
  const lineaCorrederas=permiteCorrederas?`<br>Correderas extras: <b>${money(precioMostrado(ce))}</b>`:'';
  const lineaCinta=cinta.aplica?`<br>Cinta española: <b>${cinta.metros.toFixed(2)} m · ${cinta.pct}% · separación ${cinta.sep.toFixed(2)} · ${money(precioMostrado(cinta.total))}</b>`:'';
  ...
  $('precioSeleccionado').innerHTML=`<b>${escapeHtml(nombreProducto(prod))}</b><br>${escapeHtml(variant)}<br>Medida real: ${m.toFixed(2)} m<br>Medida cobrada: ${mc.toFixed(2)} m${lineaInstalacion}${lineaSoporte}${lineaTapon}${lineaAltura}${lineaColor}${lineaLado}${lineaMotorModelo}${lineaMotor}<br>Base: <b>${money(precioMostrado(p))}</b> IVA incluido${lineaCorrederas}${lineaCinta}${lineaUnion}<br>Total unitario: <b>${money(precioMostrado(p)+precioMostrado(ce)+precioMostrado(cinta.total)+precioMostrado(soporte.total)+precioMostrado(union.total)+precioMostrado(tapon.total))}</b>`;
}
function agregarTramo(){
  ...
  const cintaEspanolaActiva=esRipplefoldEspanola(prod) && !!$('cintaEspanolaActiva')?.checked;
  const cintaEspanolaPct=cintaEspanolaActiva?($('cintaEspanolaPct')?.value||'80'):'';
  const cintaEspanolaSep=cintaEspanolaActiva?parseFloat($('cintaEspanolaSep')?.value||'0.15'):0;
  ...
  tramos.push({ubicacion:$('ubicacion').value||'Sin ubicación',medida:m,cantidad:c,producto:prod,variante:variant,colorDecorativo:requiereColorDecorativo?colorDecorativo:'',taponDecorativo:requiereColorDecorativo?taponDecorativo:'',correderasExtraPct:pct,cintaEspanolaActiva,cintaEspanolaPct,cintaEspanolaSep,alturaCordon:altura,ladoCarroMaestro:requiereLado?lado:'',motorModelo, varianteMotorizado, instalacionTradTecho, soporteMuroTrad});
  $('ubicacion').value='';$('medida').value='';$('alturaCordon').value='';$('ladoCarroMaestro').value='';$('colorDecorativo').value='';$('cantidad').value=1;$('correderasExtraPct').value='0'; if($('cintaEspanolaActiva')) $('cintaEspanolaActiva').checked=false; if($('cintaEspanolaPct')) $('cintaEspanolaPct').value='60'; if($('cintaEspanolaSep')) $('cintaEspanolaSep').value='0.15'; if($('varianteMotorizado')) $('varianteMotorizado').value='1 carro'; if($('motorModelo')) $('motorModelo').value='MC40'; render();
}
...
function calcTotals(){
  let stSin=0, stCon=0, saSin=0, saCon=0, ceSin=0, ceCon=0, cintaSin=0, cintaCon=0, sopSin=0, sopCon=0, uniSin=0, uniCon=0, tapSin=0, tapCon=0;
  for(const t of tramos){
    ...
    const cinta=cintaEspanolaInfo(t.producto,t.medida,t.cintaEspanolaActiva,t.cintaEspanolaPct,t.cintaEspanolaSep);
    ...
    stSin+=p.sin*t.cantidad; stCon+=p.con*t.cantidad; ceSin+=ce.sin*t.cantidad; ceCon+=ce.con*t.cantidad; cintaSin+=cinta.total.sin*t.cantidad; cintaCon+=cinta.total.con*t.cantidad; sopSin+=sop.total.sin*t.cantidad; sopCon+=sop.total.con*t.cantidad; uniSin+=uni.total.sin*t.cantidad; uniCon+=uni.total.con*t.cantidad; tapSin+=tap.total.sin*t.cantidad; tapCon+=tap.total.con*t.cantidad;
  }
  for(const a of accs){saSin+=a.precio.sin*a.cantidad; saCon+=a.precio.con*a.cantidad;}
  const baseSin=stSin+saSin+ceSin+cintaSin+sopSin+uniSin+tapSin; const baseCon=stCon+saCon+ceCon+cintaCon+sopCon+uniCon+tapCon;
  ...
  return {stSin,stCon,saSin,saCon,ceSin,ceCon,cintaSin,cintaCon,sopSin,sopCon,uniSin,uniCon,tapSin,tapCon,logSin,logCon,totalSin,totalCon,iva:totalCon-totalSin,total:conIva()?totalCon:totalSin};
}
function render(){ actualizarVistaPrecio(); $('tablaTramos').querySelector('tbody').innerHTML=tramos.map((t,i)=>{const mc=medidaCobradaProducto(t.producto,t.variante,t.medida), p=precioBase(t.producto,t.variante,t.medida,t.motorModelo), ce=esRipplefold(t.producto,t.variante)?precioCorrederasExtra(t.correderasExtraPct,t.medida,t.producto):{sin:0,con:0}, cinta=cintaEspanolaInfo(t.producto,t.medida,t.cintaEspanolaActiva,t.cintaEspanolaPct,t.cintaEspanolaSep), sop=soporteAutoInfo(t.producto,t.medida,t.instalacionTradTecho||'techo',t.soporteMuroTrad||'techo',t.colorDecorativo||''), uni=unionAutoInfo(t.producto,t.variante,t.medida), tap=taponDecorativoInfo(t.producto,t.taponDecorativo||'',t.colorDecorativo||''), unit=precioMostrado(p)+precioMostrado(ce)+precioMostrado(cinta.total)+precioMostrado(sop.total)+precioMostrado(uni.total)+precioMostrado(tap.total), imp=unit*t.cantidad; const unionTxt=uni.aplica?`${uni.cantidad} pza<br><span class="muted">${escapeHtml(uni.nombre)}<br>${money(precioMostrado(uni.total))}</span>`:'-'; const soporteTxt=(sop.aplica?`${sop.cantidad} pzas<br><span class="muted">${escapeHtml(sop.nombre)}<br>${money(precioMostrado(sop.total))}</span>`:'-')+(tap.aplica?`<br><br>${tap.cantidad} pzas<br><span class="muted">${escapeHtml(tap.nombre)}<br>${money(precioMostrado(tap.total))}</span>`:''); const extraTxt=(esRipplefold(t.producto,t.variante)&&t.correderasExtraPct&&t.correderasExtraPct!=='0')?`${t.correderasExtraPct}%<br><span class="muted">${money(precioMostrado(ce))}</span>`:'Sin extras'; const cintaTxt=cinta.aplica?`<br><br><b>Cinta española</b><br><span class="muted">${cinta.pct}% · ${cinta.correderasMetro} correderas/m<br>Separación ${cinta.sep.toFixed(2)} · ${cinta.metros.toFixed(2)} m<br>${money(precioMostrado(cinta.total))}</span>`:''; return `<tr><td>${escapeHtml(t.ubicacion)}<br><span class="muted">${escapeHtml(nombreProducto(t.producto))} · ${escapeHtml(t.variante)}${t.colorDecorativo?`<br>Color: ${escapeHtml(t.colorDecorativo)}`:''}${t.taponDecorativo?`<br>Tapón final: ${escapeHtml(t.taponDecorativo)}`:''}${t.motorModelo?`<br>Motor: ${escapeHtml(t.motorModelo)}`:''}${t.varianteMotorizado?`<br>Tipo/variante motorizado: ${escapeHtml(t.varianteMotorizado)}`:''}${t.instalacionTradTecho?`<br>Tipo de riel: ${t.instalacionTradTecho==='muro'?'Riel de muro':'Riel de techo'}`:''}${t.instalacionTradTecho?`<br>Soporte: ${t.soporteMuroTrad==='6'?'6"':(t.soporteMuroTrad==='3'?'3"':'Techo')}`:''}${t.ladoCarroMaestro?`<br>Carro maestro: ${escapeHtml(t.ladoCarroMaestro)}`:''}${t.alturaCordon?`<br>Altura cordón: ${Number(t.alturaCordon).toFixed(2)} m`:''}${cinta.aplica?`<br>Cinta española: ${cinta.metros.toFixed(2)} m`:''}</span></td><td>${t.medida.toFixed(2)} m</td><td>${mc.toFixed(2)} m<br><span class="muted">Base: ${money(precioMostrado(p))}</span></td><td>${extraTxt}${cintaTxt}</td><td>${soporteTxt}</td><td>${unionTxt}</td><td>${t.cantidad}</td><td class="right">${money(imp)}</td><td><button type="button" class="btn danger" data-del-tramo="${i}">X</button></td></tr>`;}).join('');
...
Cinta española: ${money(c.cintaSin||0)}
